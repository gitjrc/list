    ...
    (save-excursion
      ...
      (while (and (> go-forward-lines 0) ok)
        (unless (extent-at (setq here (point)) nil 'lnm nil 'at)
          (setq x (make-extent here here)
                g (make-glyph (format "%4d " current-line)))
          ...
          (set-extent-property x 'end-open t)
          (set-extent-property x 'begin-glyph g)
          (set-extent-property x 'lnm current-line)
          (push x line-numbers-cache)
          )
